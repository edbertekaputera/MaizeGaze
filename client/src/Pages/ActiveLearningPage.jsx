...
import { Modal, Button, Spinner, Label, TextInput, Select, Card, Tooltip, Progress } from "flowbite-react";
...
function ActiveLearningPage() {
	...
	const handleSubmit = () => {
		...
		axios.post("/api/detect/models/init_train", data).then((res) => {
			if (res.status === 200) {
				alert("Training started successfully!");
				navigate("/user/model_management");
			} else {
	...
	return (
		<div className="min-h-screen">
			<Card className="my-6 mx-4 lg:my-10 lg:mx-16 shadow-lg border xl:mb-20 pb-5">
				...
				<div className="overflow-y-auto flex flex-col 2xl:flex-row justify-between gap-y-4 p-2">
					<div className="flex flex-col pb-5 gap-4 w-full 2xl:w-3/12 2xl:pr-5 2xl:border-r-2 border-black">
						<header className="flex flex-wrap flex-row gap-2 justify-between shadow-b border-b-2 py-5 border-black">
							<h2 className="font-bold text-2xl">Training Settings</h2>
						</header>
						<section className="flex flex-col gap-2 w-full">
							<Label value="Model Name" />
							<TextInput placeholder="Enter Model Name" className="w-full" onChange={(event) => setName(event.target.value)} />
						</section>
						<section className="flex flex-col gap-2 w-full">
							<Label value="Base Model" />
							<Select
								// icon={PiFarmFill}
								id="model_selection"
								value={selectedModel}
						...
						<section className="flex flex-col gap-2 w-full">
							<Label value={`Training Data (${selected.size} data selected)`} />
							<Progress
								className="bg-custom-brown-3 text-white w-full text-center text-xl items-center align-middle"
								color="green"
								progress={selected.size > 5 ? 100 : Math.round((selected.size / 5) * 100)}
								size="xl"
								textLabel="test"
								prefix="%"
								labelProgress
						...
						<section className="flex flex-col gap-2 w-full">
							<Tooltip content={`Make sure to fill all the required fields, and select at least 5 data.`}>
								<Button
									onClick={handleSubmit}
									type="submit"
									disabled={name.trim() === "" || selected.size < 5}
									className="bg-custom-green-2 hover:bg-custom-green-1 text-white ring-inset ring-custom-green-1 w-full px-8 mt-1"
								>
					...
					<div className="flex flex-col gap-4 w-full 2xl:w-9/12 2xl:pl-5">
						<header className="flex flex-wrap flex-row gap-2 justify-between shadow-b border-b-2 py-5 border-black">
							<h2 className="font-bold text-2xl">Training Data</h2>
						</header>
						...
						<section>
							<DetectionResultTableAL results={searchFilter()} selected={selected} setSelected={setSelected} />
						</section>
					</div>
				</div>
			</Card>
