			...
			.then((res) => {
				if (res.status === 200) {
					...
					const max_tc = res.data.result.reduce((acc, value) => {
						return (acc =
							acc > value.tassel_count ? acc : value.tassel_count);
					}, 0);
					setFilter((prev) => ({
						...prev,
	...
	return (
		<div className="min-h-screen">
			...
			<Card className="my-6 mx-4 lg:my:10 lg:mx-16 shadow-lg border xl:mb-20">
				...
				<section
					className={`flex ${
						selected.size == 0 ? "justify-end" : "justify-between"
					}`}
				>
					<section
						className={`${
					...
					>
						<span className="flex flex-row items-center text-sm text-gray-500 font-medium">
							<MdOutlineClose
								size={26}
								className="mr-2 p-1 rounded-lg hover:bg-gray-200"
						...
						<div className="flex flex-row gap-2">
							...
							<Button
								color="failure"
								className="text-sm text-white font-medium shadow-md py-0"
				...
				<section className="flex flex-col xl:flex-row justify-between gap-4">
					<div className="flex flex-col md:flex-row justify-start gap-4">
						...
						<div className="flex flex-col sm:flex-row gap-4">
							<DropdownCheckbox
								...
								update_key="farm"
								icon={
									<PiFarmFill size={20} className="text-gray-400" />
								}
							/>
								...
								<Dropdown
									...
									renderTrigger={() => (
										<div className="w-full flex p-2.5 pl-3 flex-row gap-2 justify-start items-center align-middle bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg hover:ring-custom-green-1 hover:border-custom-green-1">
											<GiCorn size={20} className="text-gray-400" />
											Tassel Count
					...
					<div className="flex flex-col justify-end sm:w-1/2 sm:pr-2 xl:pr-0 xl:w-1/4">
						<TextInput
							id="Search"
