...
import { Modal, Button, Label, TextInput, Textarea, Dropdown } from "flowbite-react";
...
function CreateFarmModal({ state, setState }) {
	...
	const handleAddPatch = () => {
		const newPatch = { name: "", land_size: 1, sizeUnit: "mu" };
	...
	return (
		<>
			<MessageModal state={messageModal} setState={onCloseModal}>{`Farm '${name}' has been successfully created.`}</MessageModal>
			<Modal show={state} size="4xl" onClose={() => setState(null)} popup>
				<div className="rounded shadow-md overflow-y-auto">
					<Modal.Header>
						<div className="pl-4 pt-4 text-2xl font-bold text-gray-900 dark:text-white">Create New Farm</div>
					</Modal.Header>
					<Modal.Body>
