    ...
    const isButtonDisabled = (
      ...
      const retrieveChanges = (data) => {
        ...
        for (let newPatch of patches) {
            const dataPatch = data.patches.find(p => p.name === newPatch.name);
        ...
        for (let dataPatch of data.patches) {
            const newPatch = patches.find(p => p.name === dataPatch.name);
    ...
    const handleUpdate = () => {
        ...
        const sendData = {
            ...
            patches: (changes.patches || []).map(patch => {
                const originalPatch = farm.patches.find(p => p.name === patch.name);
                return {
                    ...patch,
